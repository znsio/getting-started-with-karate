...
Feature: Getting all the posts and albums with UserID specification
  ...
  Scenario: Get all the posts with UserID one
    ...
    When  def postsUserIdOneResult = call read('classpath:com/znsio/templates/jsonPlaceHolderTemplates.feature@t_getPosts'){query: userIdOneQuery,ecpectedStatus: 200}
  ...
  Scenario: Get all the albums with UserID one
    ...
    When def albumsUserIdOneResult = call read('classpath:com/znsio/templates/jsonPlaceHolderTemplates.feature@t_getAlbums') {query: userIdOneQuery,ecpectedStatus: 200}
